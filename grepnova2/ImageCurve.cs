...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member
    ...
    public partial class ImageCurve : UserControl
    {
        ...
        public static Bitmap ChangeChannelLevel(Bitmap image, Rectangle selectedArea, Channel channel, byte[] Levels)
        {
            ...
            Bitmap bmp = (Bitmap)image.Clone();// MakeGrayscale(image);
            ...
            Console.Out.WriteLine("WxH: {0}x{1} - Stride {2} pixelFormat {3}", bmp.Width, bmp.Height, bmpData.Stride, bmp.PixelFormat.ToString());
            // Declare an array to hold the bytes of the bitmap.
            int bytes = bmpData.Stride * bmp.Height;
            byte[] rgbValues = new byte[bytes];
            int bytesStart = 4 * selectRect.Left - 1;
            int bytesEnd = 4 * selectRect.Right + 1;
